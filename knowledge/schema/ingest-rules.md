...
- add `--report-format bundle` when the report artifact should include both a human-readable `textSummary`, path-oriented `artifacts` / `annotations`, and structured sections alongside the raw `plan`; omit it to keep the legacy plan-only JSON artifact
