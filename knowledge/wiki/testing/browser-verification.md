...
- dashboard overview and dashboard library hard refresh now have their own repo-owned probe paths too, so `/dashboard` and `/dashboard/library` can assert family-specific readiness directly and catch wrong-level app-root fallback without relying on manual repeated refresh testing
