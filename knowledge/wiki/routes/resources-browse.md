...
- Immediate detail → discover returns now separate healthy navigation from recovery behavior: same-subtree returns stay on App Router navigation so discover can reuse in-tab route state, while the route error boundary still reopens the current `/resources` URL as a full document load once inside a short session-scoped window if the route actually falls into the manual “resource library could not load” shell.
