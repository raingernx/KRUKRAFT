...
- `Warm Public Cache` waits for `/resources` to return success, then runs the repo-owned warm script before perf verification starts.
...
- The warm script now also runs a deploy-only `/resources` stability gate after the normal warm fanout. It probes the exact route in small concurrent bursts, checks the same `900ms` p95 budget that `resources_home_smoke` uses, and re-warms `/resources` before retrying if the probe still sees a cold-tail outlier. This keeps route-runtime changes out of the app path while making the post-deploy warm step prove the same home-route class that the later perf gate will measure.
- The same warm script now also warms the experiment-controlled listing pair against the exact cookie branches that the smoke suite measures: `ranking_variant=A` for the control/newest listing and `ranking_variant=B` for the treatment/recommended listing. Both routes keep repeated passes plus a concurrent burst sized to the smoke route's 5-VU ceiling, and the hot creator detail plus category listing routes keep the same repeated burst strategy, because these shapes were still alternating between pass/fail when only the generic public warm set ran.
...
- The perf runner now also allows a narrow deploy-only stabilization retry for `resources_home_smoke`: it reruns the same k6 script against the same budget, reheating only `/resources` between attempts, and still fails the job if the route cannot pass within the capped retry count. This keeps the retry logic tied to the exact noisy route instead of relaxing budgets for the whole suite.
...
- run the warm script, which primes internal caches first and then warms public route shells
...
- `/resources` is also expected to pass the deploy-only stability probe before the workflow moves on to k6. If that gate starts failing, treat it as a warmed-route instability signal, not as a reason to relax the k6 perf budget.
...
- If `PERFORMANCE_WARM_SECRET` is configured, the post-deploy warm script itself should trigger `/api/internal/performance/warm` before the route-level HTTP fanout. Do not run a second standalone internal-warm step after the public-route pass, or the perf suite will no longer start immediately after the route shells were last warmed.
- The smoke perf suite is a blocking gate: route budgets are not advisory.
- Any deploy-only stabilization retry must stay route-specific and budget-preserving. The workflow is allowed to retry `resources_home_smoke` because that route has shown fresh-instance post-deploy variance, but it must keep the original `900ms` p95 threshold and still hard-fail if the retries cannot land a clean pass.
