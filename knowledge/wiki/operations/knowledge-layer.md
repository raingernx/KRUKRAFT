...
- `--report-format bundle` upgrades that artifact from raw plan JSON to a richer report object with `textSummary`, path-oriented `artifacts`, review-focused `annotations`, structured `sections`, and the original `plan`, so CI can keep one machine-readable file for both humans and automation.
...
- add `--report-format bundle` when that artifact should carry a ready-made text summary, artifact path hints, review annotations, and sectioned JSON instead of just the raw plan payload
