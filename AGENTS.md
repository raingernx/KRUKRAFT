...
- `npm run wiki:ingest`, `npm run wiki:ingest:dry-run`, `npm run wiki:index`, `npm run wiki:lint`, `npm run wiki:stale`, `npm run wiki:coverage`, and `npm run wiki:drift` are the repo-owned operational commands for that layer; `wiki:ingest` also seeds related-page suggestions/backlinks when it creates a new wiki page
