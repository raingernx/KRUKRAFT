...
Build a new Dashboard for Krukraft without reusing the fragile route/loading
structure that caused duplicated shells and first-entry layout stacking.

Dashboard should start as an isolated prototype and remain disconnected from
...
agent to plan or critique Dashboard.

```txt
Design and plan Dashboard for Krukraft, a SaaS marketplace for digital
learning resources.
...
Dashboard must have one canonical app shell, content-only route loading, and
...
Dashboard should be planned as one dashboard product with role-aware
...
One shell exists for the entire Dashboard family.
...
Likely new dashboard domain components:
...
```txt
src/app/dev/dashboard-geometry/page.tsx
```
...
- `/dev/dashboard-geometry` renders without touching production dashboard.
