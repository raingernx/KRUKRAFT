...
async function ensureAdminDraftResource(page: Page) {
  const draftTitle = "Route-owned backend upload failure probe";
...
/F1 12 Tf
72 120 Td
(Krukraft backend upload probe) Tj
ET
endstream
endobj
xref
...
295
%%EOF`;
    const file = new File([pdf], "file-upload-backend-failure-probe.pdf", {
...
async function selectBackendFailureProbeFile(page: Page) {
  await page.waitForTimeout(1000);
  ...
  await expect(widget.getByTestId("file-upload-selected-file")).toContainText(
    "file-upload-backend-failure-probe.pdf",
  );
...
async function expectBackendFailureErrorState(
  widget: Locator,
...
test("creator create route surfaces route-owned backend upload failure copy after draft creation succeeds", async ({
  page,
}) => {
  ...
  const { widget, uploadButton } = await selectBackendFailureProbeFile(page);
  ...
  await expectBackendFailureErrorState(
    widget,
    uploadButton,
    /อัปโหลดไฟล์ไม่สำเร็จ กรุณาลองใหม่อีกครั้ง/i,
  );
...
test("admin create route surfaces route-owned backend upload failure copy after draft creation succeeds", async ({
  page,
}) => {
  const { pageErrors, consoleErrors } = collectRuntimeErrors(page);
  let uploadRequests = 0;
  ...
  await page.route("**/api/admin/resources/upload", async (route) => {
    await route.fulfill({
      ...
      contentType: "application/json",
      body: JSON.stringify({
        error: "Could not upload the file right now. Please try again.",
      }),
    });
  });
  ...
  await expectBackendFailureErrorState(
    widget,
    uploadButton,
    /Could not upload the file right now\. Please try again\./i,
  );
  expect(uploadRequests).toBe(1);
  expect(pageErrors).toEqual([]);
  expectNoUnexpectedConsoleErrors(consoleErrors, [
