...
const HARDENING_PREFIX = "Dashboard hardening probe";
...
async function seedCreatorResource(options: {
  ...
  try {
    ...
    const slug = `dashboard-hardening-${options.label.toLowerCase().replace(/[^a-z0-9]+/g, "-")}-${slugSuffix}`;
    ...
    const resource = await prisma.resource.create({
      data: {
        ...
        slug,
        description:
          "This seeded dashboard editor resource is used to harden edit-route states and verify creator flows.",
...
test("dashboard creator create editor keeps invalid external file URLs in warning state", async ({
  page,
}) => {
...
test("dashboard creator edit editor can switch from uploaded file to external URL", async ({
  page,
}) => {
  await cleanupHardeningResources();
...
test("dashboard creator edit editor persists cover reorder and preview removal", async ({
  page,
}) => {
  await cleanupHardeningResources();
  const seeded = await seedCreatorResource({
