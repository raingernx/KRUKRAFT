...
const PROBE_TITLE = "Dashboard publish probe";
...
%%EOF`;
  ...
  return {
    name: "dashboard-publish-probe.pdf",
    mimeType: "application/pdf",
    buffer: Buffer.from(pdf, "utf8"),
  };
}

test("dashboard creator create editor uploads a file and opens publish success modal", async ({
  page,
}) => {
  ...
  try {
    ...
    await page
      ...
      .fill("This resource is created by the dashboard publish probe to verify upload and publish happy paths.");
...
test("dashboard creator create editor can upload before metadata without validation overlay", async ({
  page,
}) => {
  await cleanupProbeResources();
