...
- `wiki:ingest:batch` / `wiki:ingest:batch:dry-run`: ingest a JSON batch plan for multiple raw captures/wiki stubs in one pre-validated pass and print a batch merge summary before writing when dry-run is enabled
