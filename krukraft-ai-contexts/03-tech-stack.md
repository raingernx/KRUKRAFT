...
- `.github/workflows/browser-smoke.yml` now also boots fresh `next dev` instances after the Playwright smoke bundle and runs `npm run browser:probe:dashboard`, `npm run browser:probe:pages`, and `npm run browser:probe:management`, so dashboard transitions, public/admin informational pages, admin analytics, and creator-management routes all have separate cloud browser probe paths even when local macOS Playwright launch is unreliable
