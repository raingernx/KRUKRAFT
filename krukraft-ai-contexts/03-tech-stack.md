...
- the readiness polling inside `.github/workflows/browser-smoke.yml` now keeps `curl` stderr quiet while the temporary `next dev:webpack` probe servers are still booting; the workflows still fail on a real readiness timeout or final readiness miss, but normal first-boot connection-refused noise no longer muddies otherwise healthy probe logs
