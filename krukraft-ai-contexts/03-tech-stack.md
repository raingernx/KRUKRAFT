...
- `.github/workflows/browser-smoke.yml` now also boots fresh `next dev` instances after the Playwright smoke bundle and runs both `npm run browser:probe:dashboard` and `npm run browser:probe:pages`, so dashboard transitions and the remaining public/admin informational pages have separate cloud browser probe paths even when local macOS Playwright launch is unreliable
