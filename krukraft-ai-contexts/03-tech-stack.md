...
- `scripts/warm-cache.ts` now owns one more deploy-only guard for that post-deploy path: after the normal route/internal warm fanout finishes, it probes `/resources` in short concurrent bursts and requires that probe to meet the same `900ms` p95 budget as `resources_home_smoke`, reheating `/resources` and retrying before the workflow hands control to k6 if the home route still shows a cold-tail outlier
- `scripts/run-post-deploy-perf.ts` now also owns a narrow deploy-only stabilization retry for `resources_home_smoke`: if the first k6 pass still misses the `900ms` p95 budget, it reheats only `/resources` and reruns that same route up to the capped attempt count before the rest of the suite moves on or the workflow fails
