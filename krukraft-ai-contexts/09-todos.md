...
- 2026-04-28: A softer bordered `36px` ghost-action was explored inside the `FormSection` board, but it remains an out-of-plan Figma-first candidate only. Do not treat it as an approved `Button` ladder change or canonical `ghost` recipe update until the button-size contract is deliberately reopened and runtime impact is audited.
