...
- 2026-05-01: `quiet` button posture now has a family-owned border too. Canonical Figma introduces `action/quiet-border` as a semantic stroke token derived from `primary/lift`, and runtime mirrors that through `primary-quiet-border` so quiet buttons no longer borrow the neutral `border/default` rail when a stroked edge is needed.
