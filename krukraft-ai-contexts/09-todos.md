...
- 2026-05-01: `EmptyState` action proof is now DS-backed instead of placeholder-only. The light/dark action lanes now reuse canonical `Button size=\"sm\" variant=\"soft\"` instances with a generic `Soft action` label, while the icon example stays a bounded placeholder so the board proves a real CTA without letting the action outweigh the empty-state message.
...
- 2026-04-28: The final recipe cleanup restored the canonical `Button recipes / Row action` card to an `Edit / Open` example row plus a compact `Default|Hover|Focus|Pressed|Disabled` quiet-state proof, and `DataPanelTable` now mirrors that same outline recipe posture in its light/dark foundations.
- 2026-05-01: The row-action recipe proof is now duplicated where reviewers actually need it: `Button States` carries the same quiet-state proof beside the main state matrix, and the `Button recipes / Row action` card now reuses those real quiet-state instances instead of local fake state frames in both light and dark.
