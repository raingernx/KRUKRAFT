...
- 2026-05-01: `EmptyState` action proof is now DS-backed instead of placeholder-only. The light/dark action lanes reuse canonical `Button size=\"sm\"` instances with a generic `Primary action` label, while the icon example stays a bounded placeholder so the board proves a real CTA without inventing route-specific semantics.
