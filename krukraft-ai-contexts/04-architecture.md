...
- browser-back returns from `/resources/[slug]` now get the same resources-family transition treatment as explicit link/logo navigations: the resources overlay can force a discover-shell mask even when no `beginResourcesNavigation(...)` call fired, and both that forced overlay path and the normal route-ready clearer now wait briefly for a discover-personalization ready marker before exposing `/resources` again
- the `/resources` route error boundary now shows the standard discover skeleton during its short auto-retry window instead of flashing the manual error card before the one-shot hard reload
