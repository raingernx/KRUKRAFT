...
- `/dashboard-v2/membership` is now the fifth integrated Dashboard V2 learner route: it calls `requireSession(routes.dashboardV2Membership)`, resolves subscription status/renewal/billing snapshot data through `src/services/dashboard-v2/membership.service.ts`, and keeps plan CTA decisions route-owned without exposing billing internals outside the authenticated surface
- `/dashboard-v2/settings` is now the sixth integrated Dashboard V2 learner route: it calls `requireSession(routes.dashboardV2Settings)`, resolves a read-first profile/preferences/security snapshot through `src/services/dashboard-v2/settings.service.ts`, and uses in-page anchors instead of a fake settings-menu surface
...
- `/dashboard-v2/creator/profile` is now data-backed: it calls `requireSession(routes.dashboardV2CreatorProfile)`, resolves creator access + profile data through `src/services/dashboard-v2/creator-profile.service.ts`, and renders the existing `CreatorProfileForm` inside the dashboard-v2 shell instead of routing back through the creator workspace summary surface.
- `/dashboard-v2/creator/settings` is now data-backed: it calls `requireSession(routes.dashboardV2CreatorSettings)`, resolves creator access + setup/profile-derived storefront controls through `src/services/dashboard-v2/creator-settings.service.ts`, and renders creator-specific route handoff controls instead of reusing the workspace summary panels.
...
- the root layout no longer mounts the full route-specific dashboard/resources overlay stack; `ResourcesNavigationOverlay` still lives in `src/app/resources/layout.tsx`, while the root keeps only `ResourcesEntryNavigationOverlay` for dashboard/public → `/resources` transitions. Dashboard handoffs now target canonical `/dashboard-v2/*` routes and rely on `DashboardV2Shell` plus route-owned `loading.tsx` content skeletons instead of a root-level dashboard entry overlay.
...
- the Dashboard V2 shell now also resolves a minimal optional viewer snapshot from `getCachedServerSession()` and passes only display-name/email/image/role/subscription-status fields into the topbar account menu. That optional shell snapshot is separate from route-level auth ownership: `/dashboard-v2`, `/dashboard-v2/library`, `/dashboard-v2/downloads`, `/dashboard-v2/purchases`, `/dashboard-v2/membership`, `/dashboard-v2/settings`, `/dashboard-v2/creator`, `/dashboard-v2/creator/resources`, `/dashboard-v2/creator/resources/new`, `/dashboard-v2/creator/resources/[id]`, `/dashboard-v2/creator/analytics`, `/dashboard-v2/creator/sales`, `/dashboard-v2/creator/payouts`, `/dashboard-v2/creator/profile`, and `/dashboard-v2/creator/settings` now enforce their own `requireSession(...)` gates while the route family continues moving remaining scaffold surfaces to real auth/data behavior one route at a time.
...
- Canonical `/dashboard-v2/*` entry navigations bypass the removed root-level dashboard entry overlay entirely; the real `DashboardV2Shell` plus route-owned content `loading.tsx` files are the only intended loading owners for those canonical destinations.
