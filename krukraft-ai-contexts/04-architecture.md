...
- that cached creator-public metadata reader now also fails soft for transient Prisma/Supabase infrastructure errors inside its own cache callback, returning `null` metadata instead of letting stale-cache revalidation spam Vercel runtime logs for creator pages that can otherwise recover
...
- the resources route family now keeps same-subtree discover ↔ detail hops on the App Router path again. The dedicated resilience layer stays narrow to `/resources`: `src/app/resources/error.tsx` still escalates transient recovery to a hard reopen of the current URL when it actually reaches the route error boundary, but the happy-path navigation contract no longer forces document navigation for detail returns because that tradeoff was replaying discover shell loading and personalization re-hydration on every bounce back from `/resources/[slug]`
...
- the same policy now extends into the best-effort cached readers themselves: `/resources/[slug]` review-list and related-resource caches catch recognized transient Prisma/Supabase errors inside their `unstable_cache` callbacks and return empty arrays, which keeps stale-cache revalidation from logging `revalidating cache with key ...` errors for routes that already degrade safely at the UI layer
