...
- platform settings
- the post-deploy route warm pass now burst-aligns the hot resource-detail, creator-detail, category, and listing-control routes to the same 5-VU class that the production smoke suite later measures, and it reheats the hot resource-detail plus listing-control routes again at the tail of the warm sequence so those route classes are freshest when k6 starts
