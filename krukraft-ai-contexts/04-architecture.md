...
- repo-owned scripts now operationalize the layer: `wiki:ingest` adds raw notes and optional wiki stubs, `wiki:index` regenerates the navigation file, `wiki:stale` flags old pages for review, and semantic/coverage checks now catch duplicate topics plus raw/canonical source drift
