...
- those follow-up browser-probe restart steps now also suppress normal `curl` connection-refused stderr while the temporary Webpack dev server is still booting; readiness failure semantics stay the same, but healthy first-boot polling no longer pollutes probe logs with expected startup noise
