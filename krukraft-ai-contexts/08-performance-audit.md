...
- `resources_home_smoke`
  ...
  - current mitigation: `/resources` still keeps the required warm burst and tail reheat, but the main app-level mitigation now is route decomposition itself: quick-browse shell renders immediately while discover `lead` and `collections` stream from separate cached readers instead of waiting on a monolithic discover payload, the default `Top picks` / `Trending now` rows now use a server-led public card shell instead of the generic client card wrappers, and both discover subtrees now have short best-effort timeout guards so a cold lead/collections refresh cannot hold the whole response open
- `listing_newest_smoke`
