...
- the same management/browser-probe layer now also has a creator hard-refresh shell check, so `/dashboard/creator/*` regressions that briefly fall back to the app-root loading shell can be caught as a runtime verification class instead of being left to manual repeated refreshes
