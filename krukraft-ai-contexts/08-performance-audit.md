...
- the post-deploy warm script now burst-aligns the hot `/resources`, listing-control, hot resource-detail, hot creator-detail, and category-detail routes to the same 5-VU class used by the smoke suite, and it now reheats both the listing-control pair and the hot resource-detail route as the final route-class warm steps before k6; creator public profiles also gained a Redis-backed cross-instance cache layer on top of `unstable_cache`, reducing post-deploy cold-tail variance on `creator_detail_smoke`
