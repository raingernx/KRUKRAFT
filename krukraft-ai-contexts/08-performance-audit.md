...
- the same management/browser-probe layer now also has dashboard-overview and dashboard-library hard-refresh shell checks plus creator hard-refresh shell checks, so `/dashboard`, `/dashboard/library`, and `/dashboard/creator/*` regressions that briefly fall back to the app-root loading shell can be caught as runtime verification classes instead of being left to manual repeated refreshes
