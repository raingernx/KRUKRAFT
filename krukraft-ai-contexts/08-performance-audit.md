...
- `resources_home_smoke`
  ...
  - current mitigation: `/resources` now warms as a required route with `repeat: 3` and `burst: 5`, then gets a final tail reheat with `repeat: 2` and `burst: 5` so the discover-home shell is not the stalest warmed public surface by the time k6 starts
- `listing_newest_smoke`
