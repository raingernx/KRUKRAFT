...
| `DS Primitives` | `1037:312` | 14 top-level frames spanning `Button`, `Input / Search`, `Card`, `Dropdown`, `Badge`, `Select`, and `Textarea` in paired light/dark boards | 2026-04-30 page-role split moved primitive component boards off `DS Foundations` without changing their live node ids | use this page for primitive-library coverage and future shared primitive slices such as `Avatar` and `Switch` |
