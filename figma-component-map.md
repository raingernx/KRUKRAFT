...
| `DS Components` | `1037:313` | 12 top-level frames spanning `Surface`, `FormSection`, `DataPanelTable`, `EmptyState`, `SectionHeader`, and `NotificationItem` in paired light/dark boards | 2026-04-30 page-role split moved composed shared-component boards off `DS Foundations` without changing their live node ids; the first composed follow-up slice then added paired `EmptyState / Foundations` boards plus nested light/dark source sets, the next slice added paired `SectionHeader / Foundations` boards plus nested light/dark source sets, and the 2026-05-01 admin-notification slice added paired `NotificationItem / Foundations` boards plus light/dark source sets | use this page for composed shared surfaces instead of mixing them with tokens or primitive boards |
...
| `DS Primitives` | `191:2` | `Button / Foundations / Light` | verified | 2026-04-28 control re-audit confirms the light board stays fully bound for text family/size/line-height/fill and for shell fills, strokes, and per-corner radius across the live `Button / State`, `Button / Size`, and `Button / Icon` sets. The earlier `wrapper radius debt` note was a false positive caused by zero-radius container frames and icon-instance shells, not by any non-zero local radius drift. The current foundations board now carries a dedicated `RowAction / State` companion inside `Button States`, sourced from the live `Edit / Open` posture instead of reusing `quiet`, while the recipe card keeps `Row action` as an `Edit / Open` example row plus that same `Default / Hover / Focus / Pressed / Disabled` state strip, locks `Pagination item` to the same rounded-rect `radius/sm (8px)` posture, and moves `Panel CTA` onto that same rounded-rect table posture via the calmer bounded-neutral `soft` tone. |
| `DS Primitives` | `294:29` | `Button / Foundations / Dark` | verified | same token-bound posture as the light board. The 2026-04-28 control re-audit also confirms the old dark `light recipe` subtitle drift is gone, the live foundations board now mirrors the light board structure with the same dedicated `RowAction / State` companion inside `Button States`, and no non-zero local radius debt remains in the dark `Button / State`, `Button / Size`, or `Button / Icon` component sets. |
...
| --- | --- | --- | --- | --- | --- | --- |
| `DS Primitives` | `746:510`, `553:167`, `259:92`, `1276:619`, `746:598`, `746:635`, `1329:310`, `1276:1005` | `Button` | `src/design-system/primitives/Button.tsx` | DS primitive | `mapped-manual` | Canonical file now verifies light/dark `Button / State`, `Button / Size`, and `Button / Icon` sets with the current live ids. The 2026-04-28 control re-audit cleared the stale `wrapper radius debt` claim too: the component sets no longer carry any non-zero local radius drift, and the dark subtitle drift is gone. The current Figma base keeps `primary | quiet | ghost | soft`, trials `sm=36`, and now documents recipes more explicitly: `Button States` itself now carries dedicated light/dark `RowAction / State` companions sourced from the live `Edit / Open` posture, `Row action` keeps an `Edit / Open` example row plus that same `Default / Hover / Focus / Pressed / Disabled` state strip in the recipe card, `Pagination item` shares the same rounded-rect `radius/sm (8px)` posture, and `Panel CTA` now follows that same rounded-rect posture on the calmer bounded-neutral `soft` tone instead of the older primary-pill candidate. `quiet` now carries its own `border/quiet` stroke token derived from `primary/lift` instead of borrowing `border/default`, and the shared button focus posture now promotes every canonical focus stroke to semantic `focus/ring` at `2px`. Runtime mirrors the full tone family too: `Button.tsx` adopts that quiet border plus `soft` with the lighter `bg/surface` rest state, darker `bg/inset` hover/pressed states, disabled `fg/subtle`, and a shared `focus-visible:border-2 focus-visible:border-ring` posture, while `RowActionButton` now follows the dedicated row-action state ladder directly at both `sm` and `md` sizes (`border/default` rest shell, `bg/inset` hover/pressed, `border primary` focus, `fg/subtle` disabled) and `PaginationButton size=\"md\"` continues to carry the matching rounded-rect page-item posture. The remaining runtime gap is the global `sm=32` ladder versus the Figma `sm=36` trial. |
