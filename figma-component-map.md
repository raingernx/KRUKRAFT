...
| `DS Components` | `1403:74` | `NotificationItem / Foundations / Light` | verified | 2026-05-01 notification follow-up adds a bounded admin status-shell board to `DS Components`. The light board proves only the compact notification item shell plus light `NotificationItem / Variant / Source` set (`1409:160`) with `type=success|info|warning|error` and `action=no-action|with-action`; the current shell posture is the refined compact pass: `radius/sm (8px)`, `32px` status wrappers with `20px` Phosphor-light glyphs, and an inline `20px` dismiss affordance instead of the older absolute close button. Bell-dropdown behavior, unread counts, stack choreography, and queue timing remain intentionally out of scope. The info posture now binds dedicated `support/info/soft` wrapper fills plus `support/info/base` glyph color instead of borrowing the primary family. |
| `DS Components` | `1403:99` | `NotificationItem / Foundations / Dark` | verified | same bounded contract as the light board under the dark shell, with the paired dark `NotificationItem / Variant / Source` set (`1435:2289`) and the same refined compact shell posture (`8px` radius, `20px` glyphs, inline dismiss affordance). The dark info posture now uses the same dedicated `support/info/soft` wrapper and `support/info/base` glyph contract instead of reusing `primary/*` variables. |
