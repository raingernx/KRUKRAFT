...
| `DS Foundations` | `736:178`, `746:208` | `Badge` | `src/design-system/primitives/Badge.tsx` | DS primitive | `mapped-manual` | Canonical file now provides verified `Badge / Variant / Source` and `Badge / Variant / Source / Dark` sets inside dedicated `Badge / Foundations` light/dark boards. The canonical set keeps `Badge` non-interactive and covers `neutral`, `info`, `success`, `warning`, `featured`, `destructive`, and `outline`. The 2026-04-28 shared-component re-audit confirms the boards stay fully bound for font family/text fill and for all painted fills/strokes, and the warm split still holds: `warning` stays on `bg/inset` with `support/warning/base` stroke and `support/warning/dust` text, while `featured` uses an editorial `accent/sand/wash` fill with `accent/sand/base` border/text in both light and dark. Runtime `Badge.tsx` now mirrors that canonical set directly: the earlier runtime-only aliases were remapped off the live app surface and removed from the shared primitive contract. The remaining live Figma-only gaps are explicit: the seven badge labels still use local `12/16` type overrides, and both light/dark source-set wrapper frames keep local `cornerRadius=5`. |
