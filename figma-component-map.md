...
| Foundations | `Button` | `mapped and current` | canonical file contains reusable `Button / State`, `Button / Size`, and `Button / Icon` component sets in light/dark coverage; the current Figma source now uses `primary | quiet | soft` and trials `sm=36` on the size boards while runtime still trails on `primary | quiet | ghost` and `sm=32` | keep as the best-aligned foundation slice and track the remaining runtime adoption gap explicitly |
