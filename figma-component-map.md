...
| `DS Primitives` | `1037:312` | 16 top-level frames spanning `Button`, `Input / Search`, `Card`, `Dropdown`, `Badge`, `Select`, `Textarea`, and `Avatar` in paired light/dark boards | 2026-04-30 page-role split moved primitive component boards off `DS Foundations`; the same day added canonical `Avatar / Foundations` light/dark boards plus nested `Avatar / Size` and `Avatar / Source` sets | use this page for primitive-library coverage and future shared primitive slices such as `Switch` |
| `DS Components` | `1037:313` | 6 top-level frames spanning `Surface`, `FormSection`, and `DataPanelTable` in paired light/dark boards | 2026-04-30 page-role split moved composed shared-component boards off `DS Foundations` without changing their live node ids | use this page for composed shared surfaces instead of mixing them with tokens or primitive boards |
...
| `DS Primitives` | `1063:246`, `1063:367`, `1063:317`, `1063:366`, `1063:438`, `1063:487` | `Avatar` | `src/design-system/primitives/Avatar.tsx` | DS primitive | `mapped-manual` | Canonical file now provides `Avatar / Foundations / Light` and `Avatar / Foundations / Dark` boards plus nested `Avatar / Size` and `Avatar / Source` component sets under the current live ids. The source slice locks the shared 24/32/40/56 ladder and the current runtime fallback order (image, name-derived initials, email-derived initials, explicit initials, anonymous default) while explicitly documenting the remaining gap: runtime-only 28/36/72/104 mounts still sit outside the canonical ladder, and the fallback shell keeps a narrow local brand gradient plus proportional initials sizing until avatar-specific gradient/type tokens exist. |
