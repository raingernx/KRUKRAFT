...
| `DS Components` | `1403:74` | `NotificationItem / Foundations / Light` | verified with explicit info-token gap | 2026-05-01 notification follow-up adds a bounded admin status-shell board to `DS Components`. The light board proves only the compact notification item shell plus light `NotificationItem / Variant / Source` set (`1409:160`) with `type=success|info|warning|error` and `action=no-action|with-action`; the current shell posture is the refined compact pass: `radius/sm (8px)`, `32px` status wrappers with `20px` Phosphor-light glyphs, and an inline `20px` dismiss affordance instead of the older absolute close button. Bell-dropdown behavior, unread counts, stack choreography, and queue timing remain intentionally out of scope. The current explicit gap is narrow: `info` wrappers still alias `primary/mist + primary/base` because the canonical file does not yet expose dedicated semantic support/info variables. |
| `DS Components` | `1403:99` | `NotificationItem / Foundations / Dark` | verified with explicit info-token gap | same bounded contract as the light board under the dark shell, with the paired dark `NotificationItem / Variant / Source` set (`1435:2289`) and the same refined compact shell posture (`8px` radius, `20px` glyphs, inline dismiss affordance). The same explicit gap remains: info wrappers still reuse `primary/mist + primary/base` until a dedicated support/info semantic family exists in the canonical variable file. |
