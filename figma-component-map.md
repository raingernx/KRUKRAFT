...
| `DS Foundations` | `294:29` | `Button / Foundations / Dark` | verified with narrow local radius debt and copy drift | same shell-token posture as the light board and the same wrapper-radius caveat; the current dark `Button States` subtitle still says `light recipe`, so keep that as an explicit content drift until the source file copy is corrected |
...
| Foundations | `Button` | `mapped and current` | canonical file contains reusable `Button / State`, `Button / Size`, and `Button / Icon` component sets in light/dark coverage, with `Button / Size` now normalized to the `xs/sm/md/lg` ladder from code | keep as the best-aligned foundation slice and remap node references intentionally later |
...
| --- | --- | --- | --- | --- | --- | --- |
| `DS Foundations` | `610:227`, `553:167`, `259:92`, `627:263`, `627:392`, `627:421` | `Button` | `src/design-system/primitives/Button.tsx` | DS primitive | `mapped-manual` | Canonical file now verifies light/dark `Button / State`, `Button / Size`, and `Button / Icon` sets; the 2026-04-27 audit replaced stale registry references to earlier `Button / State` ids and confirmed the current state/size/icon family is the live base. The 2026-04-27 foundations re-audit also confirmed clean semantic binding across the section shells, while recording two explicit follow-ups: wrapper-only local radius debt on the `Button / State` and `Button / Size` component-set containers, and runtime drift because Figma now expresses `quiet` / `ghost` state foreground choreography more explicitly than `src/design-system/primitives/Button.tsx` does today. |
| `DS Foundations` | `736:178`, `736:206` | `Badge` | `src/design-system/primitives/Badge.tsx` | DS primitive | `mapped-manual` | Canonical file now provides verified `Badge / Variant / Source` and `Badge / Variant / Source / Dark` sets inside dedicated `Badge / Foundations` light/dark boards. The canonical set keeps `Badge` non-interactive and covers `neutral`, `info`, `success`, `warning`, `featured`, `destructive`, and `outline`; runtime-only product/legacy variants remain outside the Figma base. The board also records one explicit token gap: badge labels still use a narrow local `12/16` type override until the typography scale gains an xs label token. |
