...
| `DS Components` | `1214:68`, `1214:93`, `1214:141`, `1214:165` | `SectionHeader` | `src/design-system/components/SectionHeader.tsx` | DS composed | `mapped-manual` | Canonical file now provides `SectionHeader / Foundations / Light` and `SectionHeader / Foundations / Dark` boards plus light/dark `SectionHeader / Variant / Source` sets under the current live ids. The 2026-05-01 composed-slice pass locks the bounded shared shell only: eyebrow, title, description, alignment, and the optional trailing actions slot through a `default | centered | with-actions | minimal` source set that mirrors runtime plus Storybook posture. Action visuals stay illustrative slot examples rather than component-owned button recipes. The remaining explicit Figma-only gap is narrow: the eyebrow tracking still stays local because the canonical file does not yet expose a reusable tracking variable. |
