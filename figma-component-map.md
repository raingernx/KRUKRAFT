...
| `DS Primitives` | `1037:312` | 20 top-level frames spanning `Button`, `Input / Search`, `Card`, `Dropdown`, `Badge`, `Select`, `Textarea`, `Avatar`, `Switch`, and `Modal` in paired light/dark boards | 2026-04-30 page-role split moved primitive component boards off `DS Foundations`; the same day added canonical `Avatar / Foundations` light/dark boards plus nested `Avatar / Size` and `Avatar / Source` sets, the follow-up slice added paired `Switch / Foundations` boards with reusable state sets, and the next heavier-primitive slice added paired `Modal / Foundations` boards plus nested `Modal / Size` sets | use this page for primitive-library coverage before reopening heavier primitive follow-up slices such as `LoadingSkeleton` |
...
| Primitives | `LoadingSkeleton` | `doc drift` | the canonical file now verifies `Select`, `Textarea`, `Avatar`, `Switch`, and `Modal`, but this remaining primitive still does not expose verified reusable nodes even though older rows once implied broader coverage | re-audit and remap it before keeping `mapped-manual` claims |
