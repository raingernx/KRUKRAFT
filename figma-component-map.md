...
| Primitives | `RevealImage`, `ToastProvider` | `code exists, figma missing` | no canonical pattern or component for either item in the current file | `RevealImage` is now intentionally deferred after inventory because its visual contract stays container-owned; keep `ToastProvider` pending until its behavior audit resolves |
