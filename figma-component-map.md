...
| `DS Primitives` | `1037:312` | 18 top-level frames spanning `Button`, `Input / Search`, `Card`, `Dropdown`, `Badge`, `Select`, `Textarea`, `Avatar`, and `Switch` in paired light/dark boards | 2026-04-30 page-role split moved primitive component boards off `DS Foundations`; the same day added canonical `Avatar / Foundations` light/dark boards plus nested `Avatar / Size` and `Avatar / Source` sets, and the follow-up slice added paired `Switch / Foundations` boards with reusable state sets | use this page for primitive-library coverage before reopening heavier primitive slices such as `Modal` or `LoadingSkeleton` |
