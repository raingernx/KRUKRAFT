...
| `DS Foundations` | `432:218`, `548:134`, `627:780`, `627:804` | `Input` | `src/design-system/primitives/Input.tsx` | DS primitive | `mapped-manual` | Canonical file now provides normalized light/dark `Input / State` and `Input / Size` sets with the current live ids. The 2026-04-28 control re-audit confirms that every field shell in those sets still uses `radius/sm (8px)` with the shared `field` height ladder, `space/16`, and caption-scale text. The remaining Figma-only gaps are explicit: the light/dark component-set wrappers keep local `cornerRadius=5`, and the search-explainer `Clear` text keeps a local `14/20` typography override. Runtime adoption has now started on the sister search primitive, but `Input.tsx` itself still carries the older larger comfortable-radius branch, so treat Figma as the approved base and `Input` as the remaining shared-field adoption gap. |
