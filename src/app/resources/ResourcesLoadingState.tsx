...
export function ResourcesLoadingState({
  ...
  if (selectedSegment !== null) {
    return <ResourcesDetailLoadingShell />;
  }

  return (
    <Suspense fallback={<ResourcesDiscoverLoadingShell />}>
      <ResourcesCollectionLoadingShell />
    </Suspense>
  );
}

function ResourcesCollectionLoadingShell() {
  const searchParams = useSearchParams();
  const category = searchParams.get("category")?.trim();

  if (!category) {
    return <ResourcesDiscoverLoadingShell />;
  }

  return <ResourcesListingLoadingShell />;
}

function ResourcesDetailLoadingShell() {
  return (
...
function ResourcesDiscoverLoadingShell() {
  return (
...
function ResourcesListingLoadingShell() {
  return (
    <div className="flex min-h-screen flex-col bg-surface-50">
      <Navbar />
