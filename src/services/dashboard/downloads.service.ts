...
export interface DashboardDownloadsData {
  ...
  downloads: DashboardDownloadItem[];
  errorTitle?: string;
  errorDescription?: string;
}

export function formatDashboardDownloadFileSize(bytes: number | null) {
  if (!bytes) {
    return "—";
  }
...
export async function getDashboardDownloadsData(input: {
  userId: string;
}): Promise<DashboardDownloadsData> {
  try {
