...
export type DashboardCreatorEditorData =
  | {
      state: "ready";
      mode: "new";
      title: string;
...
export async function getDashboardCreatorEditorData(input: {
  userId: string;
  mode: "new" | "edit";
  resourceId?: string;
  focus?: string;
}): Promise<DashboardCreatorEditorData> {
  try {
    ...
    if (!resource) {
      ...
      return {
        ...
        title: "Could not load creator editor",
        description:
          "Refresh this route to retry. The resource editor stays protected behind the dashboard creator gate.",
      };
    }
