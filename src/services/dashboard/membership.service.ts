...
type DashboardMembershipBadgeVariant = "success" | "warning" | "neutral";

export interface DashboardMembershipSummaryCard {
  label: string;
  value: string;
  detail: string;
  badgeLabel?: string;
  badgeVariant?: DashboardMembershipBadgeVariant;
}

export interface DashboardMembershipData {
  state: "ready" | "error";
  badgeLabel: string;
  badgeVariant: DashboardMembershipBadgeVariant;
  status: string;
  ...
  summaryCards: DashboardMembershipSummaryCard[];
  errorTitle?: string;
...
function getStatusBadge(status: string): {
  label: string;
  variant: DashboardMembershipBadgeVariant;
} {
  switch (status) {
    case "ACTIVE":
...
export async function getDashboardMembershipData(input: {
  userId: string;
  fallbackSubscriptionStatus?: string | null;
}): Promise<DashboardMembershipData> {
  try {
    ...
    if (!overview && !subscription) {
      return {
        ...
        primaryCtaHref: routes.dashboardMembership,
        primaryCtaLabel: "Retry",
        secondaryCtaHref: routes.dashboardPurchases,
