...
export type DashboardHomeStatKey =
  | "library"
  | "downloads"
  | "purchases"
  | "membership";

export interface DashboardHomeStatItem {
  key: DashboardHomeStatKey;
  label: string;
  value: string;
  detail: string;
  isError?: boolean;
...
export interface DashboardHomeContinueLearningItem {
  id: string;
  href: string;
  title: string;
  meta: string;
...
export interface DashboardHomeMembershipSnapshot {
  badgeLabel: string;
...
export type DashboardHomeSurfaceState<T> =
  | ReadyState<T>
  | EmptyState
  | ErrorState;
...
export interface DashboardHomeData {
  ...
  continueLearning: DashboardHomeSurfaceState<
    DashboardHomeContinueLearningItem[]
  >;
  recentActivity: DashboardHomeSurfaceState<DashboardHomeActivityItem[]>;
  membership: DashboardHomeSurfaceState<DashboardHomeMembershipSnapshot>;
}
...
function getMembershipSnapshot(params: {
  ...
  return {
    ...
    detail,
    support,
    ctaHref: routes.dashboardMembership,
    ctaLabel: isPaidPlan ? "Manage membership" : "Explore membership",
  };
}

export async function getDashboardHomeData(input: {
  userId: string;
  ...
  const stats: DashboardHomeStatItem[] = [
    {
      key: "library",
  ...
  const continueLearning: DashboardHomeData["continueLearning"] =
    purchasesResult.status !== "fulfilled"
      ? {
          status: "error",
  ...
  const recentActivity: DashboardHomeData["recentActivity"] =
    purchasesResult.status !== "fulfilled" &&
  ...
  const membership: DashboardHomeData["membership"] =
    membershipResult.status !== "fulfilled"
      ? {
          status: "error",
