...
export type DashboardCreatorResourceStatusFilter =
  | "all"
  | "DRAFT"
  | "PUBLISHED"
  | "ARCHIVED";

export type DashboardCreatorResourcePricingFilter = "all" | "free" | "paid";

export interface DashboardCreatorResourcesCategory {
  id: string;
  name: string;
}

export interface DashboardCreatorResourceItem {
  id: string;
  title: string;
  slug: string;
  href: string;
...
export interface DashboardCreatorResourcesData {
  ...
  categories: DashboardCreatorResourcesCategory[];
  totalCount: number;
  ...
  resources: DashboardCreatorResourceItem[];
  errorTitle?: string;
...
function normalizeStatus(
  ...
  switch (value) {
    ...
    case "PUBLISHED":
    case "ARCHIVED":
      return value as DashboardCreatorResourceStatusFilter;
    default:
      return "all";
  }
}
...
function normalizePricing(
  ...
  switch (firstParam(input)) {
    case "free":
    case "paid":
      return firstParam(input) as DashboardCreatorResourcePricingFilter;
    default:
      return "all";
  }
}
...
function toStatusLabel(status: string): DashboardCreatorResourceItem["status"] {
  if (status === "PUBLISHED") return "Published";
  if (status === "ARCHIVED") return "Archived";
  return "Draft";
}

export function getDashboardCreatorResourcesHref(params?: {
  status?: DashboardCreatorResourceStatusFilter;
  pricing?: DashboardCreatorResourcePricingFilter;
  sort?: CreatorResourceSort;
  categoryId?: string | null;
...
export async function getDashboardCreatorResourcesData(input: {
  userId: string;
...
}): Promise<DashboardCreatorResourcesData> {
