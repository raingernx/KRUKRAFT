...
export interface DashboardPurchasesData {
...
export function getDashboardPurchaseReference(id: string) {
  return `Order #${id.slice(0, 8).toUpperCase()}`;
}

export async function getDashboardPurchasesData(input: {
  userId: string;
}): Promise<DashboardPurchasesData> {
  try {
