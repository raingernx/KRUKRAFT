...
export type DashboardV2LibraryFilterKey =
  | "all"
  | "pdf"
  | "worksheets"
  | "templates";

export interface DashboardV2LibraryItem {
  purchaseId: string;
  purchasedAt: Date;
  id: string;
  slug: string;
...
export interface DashboardV2LibraryRecoveryItem {
  id: string;
  slug: string;
  title: string;
  authorName: string | null;
...
export type DashboardV2LibraryRecoveryState =
  | { status: "hidden" }
  | { status: "pending" }
  | { status: "confirmed"; item: DashboardV2LibraryRecoveryItem };

export interface DashboardV2LibraryData {
  state: "ready" | "empty" | "filtered-empty" | "error";
  query: string;
  filter: DashboardV2LibraryFilterKey;
  totalOwned: number;
  visibleCount: number;
  lastAddedLabel: string | null;
  continueItem: DashboardV2LibraryItem | null;
  items: DashboardV2LibraryItem[];
  recovery: DashboardV2LibraryRecoveryState;
  errorTitle?: string;
...
function matchesQuery(item: DashboardV2LibraryItem, query: string) {
  if (!query) {
    return true;
  }
...
): DashboardV2LibraryRecoveryState {
...
export function getDashboardV2LibraryHref(params?: {
  q?: string;
  filter?: DashboardV2LibraryFilterKey;
  ...
  return query ? `${routes.dashboardV2Library}?${query}` : routes.dashboardV2Library;
}

export async function getDashboardV2LibraryData(input: {
  userId: string;
  rawQuery?: string | string[] | undefined;
  rawFilter?: string | string[] | undefined;
  rawPayment?: string | string[] | undefined;
}): Promise<DashboardV2LibraryData> {
