...
export interface DashboardV2CreatorEarningsCard {
  label: string;
  value: string;
  detail: string;
}

export interface DashboardV2CreatorSaleRow {
  id: string;
  title: string;
  href: string;
  buyerLabel: string;
...
export interface DashboardV2CreatorPayoutRow {
  id: string;
  amountLabel: string;
  statusLabel: string;
  statusTone: "success" | "warning" | "neutral";
...
type DashboardV2CreatorEarningsUnavailableData =
  | {
      state: "locked";
      title: string;
      description: string;
...
export type DashboardV2CreatorEarningsData =
  | {
      state: "ready";
      cards: DashboardV2CreatorEarningsCard[];
      sales: DashboardV2CreatorSaleRow[];
      payouts: DashboardV2CreatorPayoutRow[];
    }
  | {
      state: "locked";
      title: string;
...
function getLockedEarningsData(): Extract<
  DashboardV2CreatorEarningsUnavailableData,
  { state: "locked" }
> {
  return {
    state: "locked",
...
export async function getDashboardV2CreatorEarningsData(input: {
  userId: string;
  access?: CreatorAccessState;
}): Promise<DashboardV2CreatorEarningsData> {
  try {
